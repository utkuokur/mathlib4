...
noncomputable def neighborSetedgeSpanEquiv : (neighborSet G v) ≃ (edgeSpan G v) where
  ...
  invFun := fun ⟨⟨e,he⟩,he'⟩ => by
    ...
    dsimp [neighborSet]
    rw [← G.mem_edgeSet, Sym2.other_spec he']
    exact he
...
{p : G.Walk v z} : ¬ (p.concat h).Nil := by simp [Walk.concat]
...
(h_diff : C ⟨ s(y₁,x), (G.mem_edgeSet).mp h₁ ⟩ = C ( ⟨ s(y₂,x), (G.mem_edgeSet).mp h₂ ⟩ )) : False := by
apply same_color_helper ⟨ s(y₂,x), (G.mem_edgeSet).mp h₂ ⟩ h_diff
simp
...
| ba : ∀ {u₂ u₁ v : V}, (p : G.Walk u₁ v) -> (h_prev : @at_prop V G n C α β u₁ v p)
  -> (h_adj : G.Adj u₂ u₁) -> C ⟨ s(u₂, u₁), by rw [G.mem_edgeSet]; exact h_adj ⟩  = α
  -> (h_norepeat : u₂ ∉ p.support) -> at_prop (Walk.cons h_adj p)
| ab : ∀ {u₂ u₁ v : V}, (p : G.Walk u₁ v) -> (h_prev : @at_prop V G n C α β u₁ v p )
-> (h_adj : G.Adj u₂ u₁) -> {hp : ¬ p.Nil } -> C ⟨ s(u₂, u₁), by rw [G.mem_edgeSet]; exact h_adj ⟩  = β
-> (h_norepeat : u₂ ∉ p.support) -> at_prop (Walk.cons h_adj p)
  ...
  · apply at_prop.ab
    ...
    · intro h_sup
      have h_oℓ₁ : other ≠ ℓ₁ := by
        ...
        -- have h_diff_sym2 : (f₁ : Sym2 V) ≠ (f₂ : Sym2 V):=
        exact (@lift_edge_type V G f₁ f₂ h₂).symm h_ed
...
apply at_prop.ba
repeat assumption
apply Set.notMem_subset walk_terminal_tail_support
repeat assumption
...
· assumption
· apply Set.notMem_subset walk_terminal_tail_support ; assumption
...
have h_color₁ :  C ⟨s( w₁.reverse.snd, u), h_u1 ⟩ = α := by simp only [Sym2.eq_swap] ; exact color_first_dart h_nil₁ h₁
have h_color₂ :  C ⟨s( w₂.reverse.snd, u), h_u2 ⟩ = α := by simp only [Sym2.eq_swap] ; exact color_first_dart h_nil₂ h₂
by_contra h_neq
    ...
    · simp [insert]
      ...
      · have h_vℓ₂ : v ≠ ℓ₂ := by
        ...
        have h_contra : v ∉ p.reverse.tail.support := by
          ...
          assumption
        contradiction
    · simp only [Sym2.eq_swap] ; assumption
...
-- instance [Fintype V] : Fintype (Sym2 V) :=
...
(e : (Sym2 V)) {h: e.IsDiag} (S : Set (Sym2 V))
:  fromEdgeSet (insert e S)  = fromEdgeSet S:= by
...
(eg S).edgeSet = (eg (insert e S)).edgeSet \ ( {e} : Set (Sym2 V) ):= by
simp
ext f
constructor
· simp
...
: DecidableRel ( (eg S).Adj ) := Classical.decRel ((eg S).Adj)

lemma max_degree_mono {V : Type} [ Fintype V ]
{G H : SimpleGraph V} (h_mono : G ≤ H)
...
lemma insert_back_degrees {V : Type} [ Fintype V ] [ DecidableEq V ]
{e : (Sym2 V)} {S : Finset (Sym2 V)}
: maxDegree ( (eg S)) ≤ maxDegree ( eg (insert e S))
...
def missing_color (G : SimpleGraph V) { n: ℕ } (C : (edgeSet G) → (Fin n)) (u : V)
( h_leq: (G.maxDegree + 1) ≤ n )
: ∃β : (Fin n), β ∉ (C '' (edgeSpan G u)) := by
have h_sub : ( (C '' (edgeSpan G u)).toFinset) ⊆ ((@Set.univ (Fin n)).toFinset):= by simp
have hcard : 0 < ((@Set.univ (Fin n)).toFinset).card - ((C '' (edgeSpan G u)).toFinset).card := by
  simp
  ...
  · simp
    ...
    · exact card_image_le
    · rw [degree_eq_card_edgeSpan] ; exact degree_le_maxDegree G u
  · exact h_leq
...
-- def missing_color_alt (G : SimpleGraph V) { n: ℕ }
--   (C : (edgeSet G) → (Fin n)) (u : V)
-- ( h_leq: (G.degree u + 1) ≤ n )
-- : ∃β : (Fin n), β ∉ (C '' (edgeSpan G u)) := by
-- have h_sub : ( (C '' (edgeSpan G u)).toFinset) ⊆ ((@Set.univ (Fin n)).toFinset):= by simp
-- have hcard : 0 < ((@Set.univ (Fin n)).toFinset).card - ((C '' (edgeSpan G u)).toFinset).card := by
--   simp
...
fun S => Colorable (lineGraph (eg S)) (maxDegree (eg S) + 1)

lemma empty_graph_easy : (prop_to_use (∅ : Finset (Sym2 V)) ) := by
simp [eg, prop_to_use]
apply Colorable.mono (Nat.zero_le 1)
simp
...
abbrev hC_is_valid {V : Type} {G : SimpleGraph V} {n : ℕ} (C : G.edgeSet → Fin n)
:= ∀ {e₁ e₂}, (lineGraph G).Adj e₁ e₂ → Adj ⊤ (C e₁) (C e₂)
...
lemma extending_edge_coloring_helper_alt {V : Type} [ Fintype V ] [ DecidableEq V ]
{G: SimpleGraph V} {x y: V} (n: ℕ) (α β : (Fin n))
(C: EdgeColoring (G.deleteEdges {s(x, y)}) (Fin n))
{h_xα: α ∉ (C '' (edgeSpan (G.deleteEdges {s(x, y)}) x)) }
{h_yβ: β ∉ (C '' (edgeSpan (G.deleteEdges {s(x, y)}) y)) }
{hp : ¬ @R_reach V (G.deleteEdges {s(x, y)}) n C α β y x }
: Colorable (lineGraph G) n := by
let e := s(x, y)
-- let S := (G.deleteEdges {e}).edgeFinset
let S := G.edgeFinset \ {e}
by_cases h_adj : G.Adj x y
-- swap
· have he : ¬ e.IsDiag := sorry
  have hG : G = eg (insert e S) := by
    simp [eg, S]

    -- rw [insert_edgeset_non_diag]
    ...
    · intro hf
      simp
      exact ⟨ Or.inr hf, not_isDiag_of_mem_edgeSet G hf ⟩
...
·
  have h_del_eq_G : G.deleteEdges {e} = G := by simpa [e]
  rw [h_del_eq_G] at C
  use C
  exact C.2
  --   -- rw [h_del_eq_G] at C
  --   -- exact ⟨C⟩
    ...
    -- sorry
    -- -- rw [SimpleGraph.fromEdgeSet_edgeFinset, toSet_insert]

    -- rw [S, SimpleGraph.toEdgeSet_deleteEdges, Set.insert_diff_singleton, Set.union_eq_left]
    -- exact SimpleGraph.mem_edgeSet.mpr h_adj

  -- rw [hG]
  -- have hS : eg S = G.deleteEdges {e} := by sorry
  --   -- rw [← SimpleGraph.edgeFinset_inj, SimpleGraph.fromEdgeSet_edgeFinset, S]
  -- -- rw [hS] at C h_xα h_yβ hp
  -- -- apply extending_edge_coloring_helper (S := S) (e := e) (n := n) (α := α) (β := β) (C := C) (x := x) (y := y) (hp := hp)
  -- -- · simp [S, SimpleGraph.mem_edgeFinset, SimpleGraph.deleteEdges_adj]
  -- -- · rfl
  -- -- · exact h_xα
  -- -- · exact h_yβ
  -- sorry
...
inductive FanSeq {V : Type} {G : SimpleGraph V} {n : ℕ}
  (C : EdgeColoring G (Fin n)) (x y₀ : V) : List V → Type where
| nil : FanSeq C x y₀ [y₀]
| cons {z y_i as} (h_adj : G.Adj x z)
  (h_color : C ⟨s(x, z), h_adj ⟩ ∉ C '' edgeSpan G y_i)
  (no_dup : z ∉ y_i :: as)
  (prev : FanSeq C x y₀ (y_i :: as)) : FanSeq C x y₀ (z :: y_i :: as)

def FanSeq.toList  {V : Type} {G : SimpleGraph V} {n : ℕ}
  {C : EdgeColoring G (Fin n)} {x y₀ : V} {seq: List V} (_ : FanSeq C x y₀ seq) : List V := seq

def AllFanSeqs {V : Type} [Fintype V] {G : SimpleGraph V} {n : ℕ}
    (C : EdgeColoring G (Fin n)) (x y₀ : V) : Type := {seq: List V  // Nonempty (FanSeq C x y₀ seq) }

lemma FanSeqs_length {V : Type} [Fintype V] {G : SimpleGraph V} {n : ℕ}
{C : EdgeColoring G (Fin n)} {x y₀ : V} (seq: List V) (h_seq: FanSeq C x y₀ seq)
: seq.length ≤ Fintype.card V := by
have h_nodup : seq.Nodup := by
  induction h_seq with
  | nil => simp
  | cons _ _ no_dup_prev _ a_ih =>
    simp only [List.nodup_cons]
    simp at a_ih
    exact ⟨no_dup_prev, a_ih⟩
...
lemma FanSeqs_not_nil {V : Type} {G : SimpleGraph V} {n : ℕ}
{C : EdgeColoring G (Fin n)} {x y₀ : V} {seq: List V} (h_seq: FanSeq C x y₀ seq)
: seq ≠ [] := by
...
lemma contains_y_0 {V : Type} {G : SimpleGraph V} {n : ℕ}
{C : EdgeColoring G (Fin n)} {x y₀ : V} {seq: List V} (h_seq: FanSeq C x y₀ seq)
:  y₀ ∈ seq := by
induction h_seq with
|nil=> simp
|cons _ _ _ _ a_ih => simp at * ; exact Or.inr a_ih

lemma FanSeq_head_length {V : Type} {G : SimpleGraph V} {n : ℕ}
{C : EdgeColoring G (Fin n)} {x y₀ : V} (seq: List V) (h_seq: FanSeq C x y₀ seq)
(h_length : 2 ≤ seq.length)
: (seq.head (by apply FanSeqs_not_nil h_seq )) ≠ y₀ := by
cases h_seq with
...
intro h₁
rw [h₁] at no_dup
exact no_dup (contains_y_0 as)

def FanSeq_adj {V : Type} [Fintype V] {G : SimpleGraph V} {n : ℕ}
{C : EdgeColoring G (Fin n)} {x y₀ : V} {seq: List V}
(h_seq: FanSeq C x y₀ seq)
...
lemma x_not_in_fan_seq {V : Type} [DecidableEq V] [Fintype V] {G : SimpleGraph V} {n : ℕ}
  {x y₀ : V} (h_xy : x ≠ y₀) {seq : List V} {C : EdgeColoring (G.deleteEdges {s(x, y₀)}) (Fin n)}
  (h_seq : FanSeq C x y₀ seq) : x ∉ seq := by
  intro hx_mem
  have h_adj := (FanSeq_adj h_seq x hx_mem h_xy).1
  exact G.loopless x h_adj

def FanSeq_nodup {V : Type} [Fintype V] {G : SimpleGraph V} {n : ℕ}
{C : EdgeColoring G (Fin n)} {x y₀ : V} {seq: List V} (h_seq: FanSeq C x y₀ seq)
: seq.Nodup := by
induction h_seq with
| nil => simp
| cons => apply Nodup.cons ; repeat assumption

instance fintypeListsLength (V : Type) [Fintype V] (n : ℕ) : Fintype { seq : List V // seq.length < n } := by
  induction n with
  | zero =>     exact ⟨∅, by simp ⟩
  | succ n ih =>
    ...
    apply Fintype.ofEquiv (A ⊕ B) {
      ...
      right_inv := by intro x ; simp [toFun , invFun] ; split_ifs with h' ; repeat simp
  }

instance FanSeq_fintype_helper {V : Type} [Fintype V] [DecidableEq V]
  : Fintype { seq : List V // seq.length < Fintype.card V + 1 } := fintypeListsLength V (Fintype.card V + 1)

noncomputable instance FanSeqs_finite {V : Type} [Fintype V]  {G : SimpleGraph V} {n : ℕ}
{C : EdgeColoring G (Fin n)} {x y₀ : V} : Finite (AllFanSeqs C x y₀)  := by
apply Fintype.finite
let func: (AllFanSeqs C x y₀) → {seq: List V // seq.length < Fintype.card V + 1} := λ s
=> ⟨ s.1, by apply Nat.lt_succ_of_le ; exact FanSeqs_length s.1 (Nonempty.some s.prop)⟩
...
instance FanSeqs_nonempty {V : Type} [Fintype V] [DecidableEq V] {G : SimpleGraph V} {n : ℕ}
  (C : EdgeColoring G (Fin n)) (x y₀ : V)
  : Nonempty (AllFanSeqs C x y₀) := by use [y₀], FanSeq.nil

def FanSeqs_max {V : Type} [Fintype V] [DecidableEq V] {G : SimpleGraph V} {n : ℕ}
  (C : EdgeColoring G (Fin n)) (x y₀ : V)
  : ∃ seq : (AllFanSeqs C x y₀) , ∀ w : (AllFanSeqs C x y₀), w.1.length ≤ seq.1.length := by
  apply Finite.exists_max

def next_elem {V : Type} [DecidableEq V] (z: V) (seq : List V) (hz : z ∈ seq)
(h_not_last : z ≠ seq.head (List.ne_nil_of_mem hz) )
: V := match seq with
| [] => by simp at hz
| a :: as =>
match as with
| [] => by simp at hz h_not_last ; contradiction
| b :: bs => by
  simp at h_not_last
  simp [h_not_last] at hz
  if h_z_is_b : z = b then exact a
  else
  exact next_elem z (b :: bs) (by simpa) h_z_is_b

def next_elem_prop {V : Type} [DecidableEq V] { z a: V } {as : List V}
(hz_as : z ∈ as) (h_not_last : z ≠ as.head (List.ne_nil_of_mem hz_as))
{hza : z ∈ a :: as} {h_not_a : z ≠ a}
: next_elem z (a :: as) hza h_not_a = next_elem z as hz_as h_not_last:= by
cases as with
| nil => rfl
| cons =>
simp at h_not_last
simp [ h_not_last, next_elem ]

def next_in_seq {V : Type} [DecidableEq V] (z: V) (seq : List V) {hz : z ∈ seq}
{h_not_last : z ≠ seq.head (List.ne_nil_of_mem hz)}
: (next_elem z seq hz h_not_last) ∈ seq  := by
induction seq with
| nil => simp at hz
| cons a as a_ih =>
cases as with
| nil =>  simp at hz ; simp [hz] at h_not_last
| cons b bs =>
by_cases hp : z = b
· simp [hp, next_elem]
· simp at h_not_last
  have h₁ : z ∈ b :: bs := mem_of_ne_of_mem h_not_last hz
  have h₂ : z ≠ (b :: bs).head (by simp):= by simp ; assumption
  rw [next_elem_prop h₁ h₂]
  apply List.mem_cons_of_mem
  exact @a_ih h₁ h₂

def next_not_y_0 {V : Type} [DecidableEq V] {G : SimpleGraph V} {n : ℕ}
{x y₀ z: V } {seq : List V}
{ C : EdgeColoring (G.deleteEdges { s(x,y₀) }) (Fin n) }
( h_seq : FanSeq C x y₀ seq )
{ hz : z ∈ seq } {h_not_last : z ≠ seq.head (List.ne_nil_of_mem hz)}
: (next_elem z seq hz h_not_last) ≠ y₀   := by
induction h_seq with
| nil => simp at hz ; simp [hz] at h_not_last
| @cons a b bs h_adj missing nodup h_seq_prev ih =>
by_cases hp : z = b
· simp [hp, next_elem]
  intro h_contr
  rw [h_contr] at nodup
  exact nodup (contains_y_0 h_seq_prev)
· simp at h_not_last
  have h₁ : z ∈ b :: bs := mem_of_ne_of_mem h_not_last hz
  rw [next_elem_prop h₁ hp ]
  exact @ih h₁ hp

def next_in_edgeset {V : Type} [DecidableEq V] [Fintype V] {G : SimpleGraph V} {n : ℕ}
 {x y₀ z: V } (h_xy : x ≠ y₀) {seq : List V}
{ C : EdgeColoring (G.deleteEdges { s(x,y₀) }) (Fin n) }
( h_seq : FanSeq C x y₀ seq )
{ hz : z ∈ seq } {h_not_last : z ≠ seq.head (List.ne_nil_of_mem hz)}
: s(x, next_elem z seq hz h_not_last) ∈ (G.deleteEdges {s(x, y₀)}).edgeSet := by
let w := next_elem z seq hz h_not_last
have hw : w ∈ seq := by simp [w] ; exact next_in_seq z seq
have hwy₀ : w ≠ y₀  := by simp [w] ; exact next_not_y_0 h_seq
simp
constructor
· exact (FanSeq_adj h_seq w hw hwy₀).1
· exact ⟨ hwy₀ , by simp [h_xy] ⟩

def next_missing_color {V : Type} [DecidableEq V] [Fintype V]
{G : SimpleGraph V} {n : ℕ}
{x y₀ z: V } (h_xy : x ≠ y₀) {seq : List V}
{ C : EdgeColoring (G.deleteEdges { s(x,y₀) }) (Fin n) }
( h_seq : FanSeq C x y₀ seq )
(hz : z ∈ seq ) (h_not_last : z ≠ seq.head (List.ne_nil_of_mem hz))
: C ⟨ s(x, (next_elem z seq hz h_not_last) ) , next_in_edgeset h_xy h_seq ⟩
∉ C '' edgeSpan (G.deleteEdges { s(x,y₀) }) z  := by
induction h_seq with
| nil => simp at hz ; simp [hz] at h_not_last
| @cons a b bs ad missing d h_prev ih =>
by_cases hp : z = b
· have h₁ : next_elem z (a :: b :: bs) hz h_not_last = a := by
    simp [hp, next_elem]
  simp only [h₁]
  rw [<- hp ] at missing
  exact missing
· simp at h_not_last
  have h₁ : z ∈ b :: bs := mem_of_ne_of_mem h_not_last hz
  simp only [next_elem_prop h₁ hp ]
  apply ih

abbrev unchanged {V : Type} (x: V) (seq : List V)
 : Sym2 V → Prop := fun f => ∀ u, u ∈ seq → f ≠ s(x,u)

-- abbrev unchanged_alt {V : Type} [DecidableEq V]
-- (x: V) (seq : List V) (f: Sym2 V)
--  : Option V := seq.find? (fun u => decide ( f = s(x, u) ))

lemma find_some_z {V : Type} [DecidableEq V] [Fintype V] {G : SimpleGraph V}
{x y₀ z: V} {seq : List V} {n : ℕ}
{ C : EdgeColoring (G.deleteEdges { s(x,y₀) }) (Fin n) }
( h_seq : FanSeq C x y₀ seq ) {f : Sym2 V}
( hf: f ∈ (G.deleteEdges { s(x, (seq.head (FanSeqs_not_nil h_seq))) }).edgeSet )
( h_find : seq.find? (fun u => decide (f = s(x, u))) = some z )
:  z ≠ seq.head (FanSeqs_not_nil h_seq) := by
let p:= (fun u => decide ( f = s(x, u) ))
have h_prop_bool : p z = true := (List.find?_some h_find)
have h_prop_prop : f = s(x, z) := by
  simpa [p] using h_prop_bool
intro h_eq_head
rw [h_eq_head] at h_prop_prop
simp [edgeSet_deleteEdges] at hf
exact hf.2 h_prop_prop

lemma find_none {V : Type} [DecidableEq V] [Fintype V] {G : SimpleGraph V}
{x y₀: V} {seq : List V} {n : ℕ}
{ C : EdgeColoring (G.deleteEdges { s(x,y₀) }) (Fin n) }
( h_seq : FanSeq C x y₀ seq ) {f : Sym2 V}
( hf: f ∈ (G.deleteEdges { s(x, (seq.head (FanSeqs_not_nil h_seq))) }).edgeSet )
( h_find : seq.find? (fun u => decide (f = s(x, u))) = none )
: f ∈ (G.deleteEdges {s(x, y₀)}).edgeSet := by
let p:= (fun u => decide ( f = s(x, u) ))
have this := List.find?_eq_none.mp h_find
have h_unchanged : ∀ u ∈ seq, f ≠ s(x, u) := by simp at this ; exact this
specialize h_unchanged y₀ (contains_y_0 h_seq)
simp [deleteEdges] at *
exact ⟨ hf.1, h_unchanged ⟩

lemma eq_of_find_some {V : Type} [DecidableEq V] {x z : V} {f : Sym2 V} {seq : List V}
  (h_find : seq.find? (fun u => decide (f = s(x, u))) = some z) :
  f = s(x, z) := by
  let p := fun u => decide (f = s(x, u))
  have h_prop_bool : p z = true := (List.find?_some h_find)
...
lemma next_elem_inj {V : Type} [DecidableEq V] {seq : List V} (h_nodup : List.Nodup seq)
{z₁ z₂ : V} {hz₁ : z₁ ∈ seq} {h_not_last₁ : z₁ ≠ seq.head (List.ne_nil_of_mem hz₁)}
{hz₂ : z₂ ∈ seq} {h_not_last₂ : z₂ ≠ seq.head (List.ne_nil_of_mem hz₂)}
(h_eq : next_elem z₁ seq hz₁ h_not_last₁ = next_elem z₂ seq hz₂ h_not_last₂) : z₁ = z₂ := by
induction seq with
  | nil => simp at hz₁
  | cons a as ih =>
    cases as with
    | nil => simp at hz₁; contradiction
    | cons b bs =>
      simp only [List.head_cons, next_elem] at h_not_last₁ h_not_last₂ h_eq
      have ha_notin_as : a ∉ b :: bs := (List.nodup_cons.mp h_nodup).1
      have h₁_mem_tail : z₁ ∈ b :: bs := (List.mem_cons.mp hz₁).resolve_left h_not_last₁
      have h₂_mem_tail : z₂ ∈ b :: bs := (List.mem_cons.mp hz₂).resolve_left h_not_last₂
      split_ifs at h_eq with h₁ h₂
      · simp [h₁,h₂]
      · by_contra
        exact ha_notin_as (h_eq ▸ @next_in_seq V _ z₂ (b::bs) h₂_mem_tail h₂)

      · by_contra
        exact ha_notin_as (h_eq.symm ▸ @next_in_seq V _ z₁ (b::bs) h₁_mem_tail h₁)
      · exact ih (List.nodup_cons.mp h_nodup).2 h_eq

lemma map_rel_aux_proof {V : Type} [DecidableEq V] [Fintype V]
{G : SimpleGraph V} {n : ℕ} {x y₀: V} (h_xy : x ≠ y₀)
(C : EdgeColoring (G.deleteEdges { s(x,y₀) }) (Fin n))
(seq : List V) (h_seq: FanSeq C x y₀ seq)
(f₁ f₂ : (G.deleteEdges {s(x, seq.head (FanSeqs_not_nil h_seq))}).edgeSet)
(h_int : (lineGraph (G.deleteEdges {s(x, seq.head (FanSeqs_not_nil h_seq) )})).Adj f₁ f₂)
(h_find₁ : seq.find? (fun u => decide (↑f₁ = s(x, u))) = none)
(z : V) (h_find₂ : seq.find? (fun u => decide (↑f₂ = s(x, u))) = some z) :
C ⟨f₁, find_none h_seq f₁.2 h_find₁⟩ ≠
C ⟨s(x, next_elem z seq (List.mem_of_find?_eq_some h_find₂) (find_some_z h_seq f₂.2 h_find₂)),
    next_in_edgeset h_xy h_seq⟩ := by
  let p₁ := (fun u => decide ( f₁ = s(x, u) ))
  have this := List.find?_eq_none.mp h_find₁
  have h_unchanged : ∀ u ∈ seq, f₁.1 ≠ s(x, u) := by simp at this ; exact this
  have h_prop_prop : f₂ = s(x, z) := eq_of_find_some h_find₂
  have f_10 : f₁.1 ∈ (G.deleteEdges {s(x, y₀)}).edgeSet := find_none h_seq f₁.2 h_find₁
  simp [lineGraph, Adjacent, AdjacentAt] at h_int
  obtain ⟨ v, hv ⟩  := h_int.1
  by_cases hux : v = x
  · rw [hux] at hv
    apply C.map_rel'
    simp [lineGraph, Adjacent, AdjacentAt]
    constructor
    · use x; simp; exact hv.1
    · intro h_contra
      rw [h_contra] at h_unchanged
      have hz := List.mem_of_find?_eq_some h_find₂
      have h_z_not_head := find_some_z h_seq f₂.2 h_find₂
      specialize h_unchanged (next_elem z seq hz h_z_not_head)
      simp at h_unchanged
      exact h_unchanged (next_in_seq z seq)
  · simp [h_prop_prop, hux] at hv
    intro h_contra
    have hz := List.mem_of_find?_eq_some h_find₂
    have h_z_not_head := find_some_z h_seq f₂.2 h_find₂
    have missing := next_missing_color h_xy h_seq hz h_z_not_head
    apply missing
    rw [<- h_contra]
    use ⟨ f₁, f_10 ⟩
    simp [<-hv.2]
...
def shift_recursive {V : Type} [DecidableEq V] [Fintype V]
  {G : SimpleGraph V} {n : ℕ} (x y₀: V) (h_xy : x ≠ y₀)
  (C : EdgeColoring (G.deleteEdges { s(x,y₀) }) (Fin n))
  {seq : List V} (h_seq: FanSeq C x y₀ seq)
  : EdgeColoring (G.deleteEdges { s(x, (seq.head (FanSeqs_not_nil h_seq)) ) }) (Fin n) := by
  refine {toFun :=?_, map_rel' :=?_}
  · intro ⟨ f, hf ⟩
    let p : V -> Bool := fun u => decide ( f = s(x, u) )
    match h_find: seq.find? (fun u => decide ( f = s(x, u) )) with
    | none =>
      exact C ⟨ f, by apply find_none h_seq hf h_find ⟩
    | some z =>
      have hz : z ∈ seq := List.mem_of_find?_eq_some h_find
      have h_z_not_head : z ≠ seq.head (FanSeqs_not_nil h_seq)
      := find_some_z h_seq hf h_find
      let w := next_elem z seq hz h_z_not_head
      exact C ⟨ s(x,w), next_in_edgeset h_xy h_seq ⟩
  ·
    intro f₁ f₂ h_int
    simp
    split
    · case' h_1 h_find₁ =>
      split
      · apply C.map_rel'
        simp [lineGraph, Adjacent, AdjacentAt] at *
        exact ⟨ h_int.1, @lift_edge_type V _ f₁ f₂ h_int.2 ⟩
      · case' h_2 z h_find₂ =>
        exact map_rel_aux_proof h_xy C seq h_seq f₁ f₂ h_int h_find₁ z h_find₂

    · case' h_2 z₁ h_find₁ =>
      split
      · case' h_1 z₁ h_find₂ =>
        rw [eq_comm]
        exact map_rel_aux_proof h_xy C seq h_seq f₂ f₁ h_int.symm h_find₂ z₁ h_find₁

      · case' h_2 z₂ h_find₂ =>
        apply C.map_rel'
        simp [lineGraph, Adjacent, AdjacentAt]
        have hf₁_eq : f₁ = s(x, z₁) := eq_of_find_some h_find₁
        have hf₂_eq : f₂ = s(x, z₂) := eq_of_find_some h_find₂
        have hz_ne : z₁ ≠ z₂ := by
          intro h_z_eq
          rw [h_z_eq] at hf₁_eq
          exact h_int.2 (Subtype.ext (hf₁_eq.trans hf₂_eq.symm))
        have hz₁_mem := List.mem_of_find?_eq_some h_find₁
        have hz₁_not_head := find_some_z h_seq f₁.2 h_find₁
        have hz₂_mem := List.mem_of_find?_eq_some h_find₂
        have hz₂_not_head := find_some_z h_seq f₂.2 h_find₂

        constructor
        · intro h_w_eq
          have h_nodup := FanSeq_nodup h_seq
          exact hz_ne (next_elem_inj h_nodup h_w_eq)
        · intro h_x_w₂
          have h_w₁_in_seq : next_elem z₁ seq hz₁_mem hz₁_not_head ∈ seq := next_in_seq z₁ seq
          have hx_notin_seq : x ∉ seq := x_not_in_fan_seq h_xy h_seq
          intro h_w1_eq_x
          rw [h_w1_eq_x] at h_w₁_in_seq
          exact hx_notin_seq h_w₁_in_seq
...
have delta_comp :  Δ₁ ≤ Δ_host_1   := by
  ...
  exact (insert_back_degrees).1
change Colorable (lineGraph G_host ) Δ_host_1
by_cases h_diag : e.IsDiag
· -- e is diagonal, and eliminated while forming G_host
  have h_loop : G_host = G_S := by
    simp [G_host, G_S, eg, @insert_edgeset_diag V _ _ e h_diag (toSet S)]
  have h_loop_deg : Δ_host_1 = Δ₁ := by
    simp [Δ_host_1, Δ₁, G_host, G_S, eg,
      @insert_edgeset_diag V _ _ e h_diag (toSet S)]
...
· -- e is non-diagonal, and is an edge of G_host
  ...
  have h_S_host : G_host.deleteEdges {s(x,y₀)} = G_S := by
    simp [G_S, G_host, <-h_exy, deleteEdges, <-fromEdgeSet_sdiff, h_eS]

  have C₀: EdgeColoring (G_host.deleteEdges {s(x,y₀)}) (Fin Δ₁) := by
    simp only [h_S_host]
    use C

  obtain ⟨ seq', h_max ⟩ := FanSeqs_max C₀ x y₀
  rcases seq' with ⟨ seq, h_seq' ⟩
  obtain ⟨h_seq⟩ := h_seq'

  let y_k := seq.head (FanSeqs_not_nil h_seq)
  let G_k := G_host.deleteEdges { s(x, y_k ) }
  let C_k : EdgeColoring G_k (Fin Δ₁) := shift_recursive x y₀ h_xy.symm C₀ h_seq
  obtain ⟨ α, hα ⟩ := missing_color G_S C x (by simp [Δ₁])
  obtain ⟨ β, hβ ⟩ := missing_color G_S C y_k (by simp [Δ₁])

  -- have h_y₀y_k : y_k ≠ y₀ := by simp [y_k] ; apply FanSeq_head_length seq h_seq ;

  have hαC_k : α ∉ C_k '' edgeSpan G_k x := by
    sorry
  have hβC_k : β ∉ C_k '' edgeSpan G_k y_k := by

    sorry
  by_cases h_reach : @R_reach V G_k Δ₁ C_k α β y_k x
  ·

    sorry
  ·
    simp only [G_host]
    apply extending_edge_coloring_helper_alt Δ₁ α β C_k
...
def line_embed {V : Type} {G : SimpleGraph V} {G' : SimpleGraph V}
(h₁ : G = G') : (lineGraph G) ↪g (lineGraph G')  := by
  subst h₁
  refine { toFun := id , inj' := Function.injective_id , map_rel_iff' := (by simp) }

--TRY: def line_embed {V : Type} {G : SimpleGraph V}
-- {V' : Type} {G' : SimpleGraph V'}
-- (h₁ : G ↪g G') : (lineGraph G) ↪g (lineGraph G')

lemma line_G_colorable {V : Type} [Fintype V] [DecidableEq V]
  (G : SimpleGraph V) [DecidableRel G.Adj] :
  Colorable (lineGraph G) (maxDegree G + 1) := by
  have h: (prop_to_use (G.edgeSet).toFinset) := by apply line_G_colorable_induced
  simp [eg, prop_to_use] at h
  have h₁ : G = (eg G.edgeSet.toFinset) := by simp [eg]
  have h₂ : (lineGraph G) →g (lineGraph (eg G.edgeSet.toFinset)) := line_embed h₁
  apply SimpleGraph.Colorable.of_hom h₂
  exact h
